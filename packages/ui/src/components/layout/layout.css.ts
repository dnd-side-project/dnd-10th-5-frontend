...
export const paddingY = style({
  paddingTop: dynamicVars.paddingY,
  paddingBottom: dynamicVars.paddingY,
});
...
type LayoutDynamicVariants = {
  [K in keyof typeof dynamicVars]?: number | string;
};

export const layoutEnumVariants = recipe({
  variants: {
    position,
    overflow,
    overflowX,
...
type LayoutEnumVariants = Exclude<
  RecipeVariants<typeof layoutEnumVariants>,
  undefined
>;

export type LayoutVariants = LayoutDynamicVariants & LayoutEnumVariants;
